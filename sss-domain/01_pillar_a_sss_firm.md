---
artifact_id: "IKIS-SSS-2026-002"
type: "sss_initiative"
title: "Pillar A: SSS Firm: SSS Firm — Structuring Engine"
status: "validated"
confidence: "high"
pillar: "A"
portfolio_entity: "SSS"
authors:
  - "marshallwmorrison"
  - "ai-agent"
owner: "marshallwmorrison"
created_date: "2026-07-05"
last_reviewed: "2026-07-05"
review_cycle_months: 3
provenance:
  source_artifacts:
    - "IKIS-SRC-2026-001"
  validation_method: "expert_review"
  validated_by:
    - "marshallwmorrison"
  validated_date: "2026-07-05"
tags:
  - "pillar-a"
  - "sss-firm"
  - "advisory"
  - "structuring"
related_artifacts:
  - "IKIS-PFE-2026-001"
  - "IKIS-SSS-2026-003"
  - "IKIS-SSS-2026-004"
governance:
  classification: "internal"
  escalation_required: false
  legal_reviewed: true
---

# Pillar A: SSS Firm: SSS Firm — Structuring Engine

> **The advisory and structuring engine of the SSS platform. Revenue derives from advisory fees, management fees on AUM, and carried interest on realized gains.**

---

## 1. Overview

Pillar A: SSS Firm is Signature Sovereign Solutions, LLC itself — the Florida-registered operational entity that structures sovereign-grade trade corridors and premium CPG investments. It generates revenue through three streams:

1. **Advisory Fees** — Fees for structuring, advisory, and consulting services
2. **Management Fees** — 1.5% annual fee on assets under management (AUM)
3. **Carried Interest** — 20% of realized gains on portfolio investments

## 2. Entity Details

| Attribute | Value |
|-----------|-------|
| **Legal Name** | Signature Sovereign Solutions, LLC |
| **Jurisdiction** | Florida, United States |
| **Status** | Active (registered with Florida Division of Corporations) |
| **Registered Office** | Miami, FL |
| **Website** | signaturesovereignsolutions.com |
| **Key Personnel** | Christopher Konopka (CEO), Marshall W. Morrison (COO) |

## 3. Revenue Model

### 3.1 Revenue Stream Decomposition

| Stream | Y1 | Y3 | Y5 | Dependency |
|--------|-----|-----|-----|------------|
| Advisory Fees | ~$3M | ~$10M | ~$19.2M | Mandate origination |
| Management Fees (1.5% AUM) | ~$1.5M | ~$4.5M | ~$7.5M | AUM growth |
| Carried Interest (20%) | $0 | ~$1-2M | ~$5M+ | Realized exits in B + C |
| **Total Revenue** | **~$3.66M** | **~$12M** | **~$24.16M** | |

### 3.2 Key Financial Metrics

| Metric | Y1 | Y5 | Notes |
|--------|-----|-----|-------|
| Revenue | $3.66M | $24.16M | Aggressive but plausible if mandates close |
| EBITDA Margin | 7.1% | 56.5% | High but achievable for asset-light advisory |
| Mandates | 6 | 24 | Requires strong pipeline conversion |
| AUM | $100M | $500M | Depends on B + C closings |
| Enterprise Value | — | $109.25M | At 8x EBITDA — within advisory range (6-10x) |
| MOIC | — | 9.1x | Sponsor economics — highly leveraged to carry |
| IRR | — | 55.5% | Achievable only if carry realizes on schedule |

### 3.3 Critical Dependencies

- Year 1 revenue requires closing both the AIA anchor ($25M) and Corleone Series A ($16M)
- 56.5% EBITDA margin at Year 5 assumes minimal headcount scaling vs. fee scaling
- 9.1x MOIC is sponsor economics (founding team return), not investor economics

## 4. Key Risks

| Risk | Probability | Impact | Mitigation |
|------|------------|--------|------------|
| Mandate origination below plan | Medium | High | Three-way differentiation narrative |
| Key-person event (Konopka/Morrison) | Low | Fatal | No succession plan documented |
| Regulatory action (unregistered securities) | Low-Medium | Fatal | 506(c) structure planned |
| Pillar B: Afro-Indian-American Strategic Minerals and Capital Corridor (AIA-SMCC) failure | High | Crippling | Diversified revenue streams |
| Pillar C: Corleone Enterprise Holdings failure | Medium-High | Major | Diversified revenue streams |

## 5. Related Artifacts

- **Pillar B: Afro-Indian-American Strategic Minerals and Capital Corridor (AIA-SMCC)**: `02_pillar_b_aia_corridor.md`
- **Pillar C: Corleone Enterprise Holdings**: `03_pillar_c_corleone.md`
- **Strategic Thesis**: `08_strategic_thesis.md`

## 6. Handoff Notes

- **Status**: Operational, registered, active
- **Immediate Need**: Close first mandates (AIA anchor + Corleone Series A)
- **Succession Planning**: Not documented — gap identified

---

*Last updated: 2026-07-05*  
*Status: Validated*  
*Artifact ID: IKIS-SSS-2026-002*  
*Owner: marshallwmorrison*
